...
const ITEM_COLLAPSED_WIDTH = 22;
const ITEM_EXPANDED_WIDTH = 56;
const TRACK_COLLAPSED_WIDTH = 10;
const TRACK_EXPANDED_WIDTH = 34;
const COLUMN_GAP = 12;
...
export function DailyFlowChart({ trend, period, openingBalance }: { trend: TrendPoint[]; period: Period; openingBalance: number }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-[24px] border border-white/8 bg-white/[0.028] p-5">
        <div className="mb-4 flex min-h-6 items-center justify-between gap-3 text-[13px] leading-5">
          {selectedPoint ? (
            <>
            ...
            <>
              <p className="text-white/44">Mes actual</p>
              <p className="text-white/38">Cálculo de beneficios</p>
            </>
          )}
        </div>
          ...
          <div className="flex items-end px-1" style={{ gap: `${COLUMN_GAP}px`, width: `${chartWidth}px` }}>
            {trend.map((point) => {
              ...
              return (
                ...
                >
                  <div
                    className={`relative overflow-visible rounded-[20px] border bg-white/[0.03] transition-all duration-250 ease-out ${
                      isSelected
                  ...
                  >
                    ...
                    {hasMovement ? (
                      <>
                        {isPositive ? (
                          <div
                            className={`absolute left-1/2 bottom-1/2 -translate-x-1/2 rounded-full bg-[linear-gradient(180deg,rgba(160,208,255,0.98),rgba(77,124,255,0.6))] shadow-[0_10px_24px_rgba(76,124,255,0.24)] transition-all duration-250 ${isSelected ? 'w-[20px]' : 'w-[10px]'}`}
                        ...
                        {isNegative ? (
                          <div
                            className={`absolute left-1/2 top-1/2 -translate-x-1/2 rounded-full bg-[linear-gradient(180deg,rgba(255,180,199,0.98),rgba(255,86,118,0.6))] shadow-[0_10px_24px_rgba(255,88,121,0.2)] transition-all duration-250 ${isSelected ? 'w-[20px]' : 'w-[10px]'}`}
                            style={{ height: `${barHeight}px` }}
                          />
                        ) : null}
                      </>
                  ...
                  <span
                    ...
                    style={{ width: `${isSelected ? ITEM_EXPANDED_WIDTH : 28}px` }}
                  >
...
function getLabelTop(net: number) {
  if (net >= 0) {
    return ZERO_LINE_Y + 10;
  }

  return ZERO_LINE_Y - LABEL_PILL_HEIGHT - 10;
}
