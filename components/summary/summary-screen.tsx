...
export function SummaryScreen({ data }: { data: DashboardData }) {
  const maxTrendBalance = Math.max(...data.trend.map((point) => point.runningBalance), data.summary.openingBalance);
  const maxExpenseCategory = Math.max(...data.expenseCategories.map((item) => item.amount), 1);

  return (
    <div className="space-y-5">
      <PageHeader
        eyebrow="App Contabilidad Diaria"
        title="Control claro, sin ruido"
        description="Una vista rápida del mes para entender qué entra, qué sale y dónde se está moviendo tu dinero."
        action={<div className="badge-pill">{data.summary.monthLabel}</div>}
      />
